...
def remove_permission_role(data, permission_to_check="ROLE_MOD"):
    ...
    if verify_session_file(session_id):
        ...
        if check_permission_user(session_roles, session_org_id, session_username, permission_to_check):
            ...
            try:
                ...
                if "roles" in current_acl and role in current_acl["roles"]:
                    ...
                    permissions = current_acl["roles"][role].get("permissions", [])
                    if permission in permissions:
                        permissions.remove(permission)
                        current_acl["roles"][role]["permissios"] = permissions
                    else:
                        return f"Permission '{permission}' already exists in role '{role}'", 409
                else:
